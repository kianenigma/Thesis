...
Altering chain topology brings even more radical changes to the original idea of blockchain. While
...
Having all these parameters fixed, we can focus on the block building part, namely executing each
...
\nomenclature{Concurrency Avoidance}{A scheme in contrast to concurrency control in which no synchronization is used between the threads}
...
Both locking and transactional memory results in a sizeable overhead while authoring. This is mostly
...
\nomenclature{Taintable State}{Special HashMap-like data structure that assigns a taint to each key}
...
The worker side of the processing (with slight simplifications) is as follows:
...
A number of noteworthy remarks exist on the baseline algorithm:
...
\textbf{Orphan Transactions}. An orphan transaction is a transaction that has already been forwarded
...
Given the deterministic execution of all types of transactions in our concurrent system, we conclude
...
We use this pseudo-static information to our benefit, by proposing a static annotation to be added
