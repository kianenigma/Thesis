...
As mentioned, blockchains can be seen, in a very broad way, as \textit{decentralized state machines
...
\begin{itemize}
	...
	the word lock free and wait free.}wait-free. In an abstract level, there is no waiting involved
	in this model. Any access to a key in the state will either \textit{immediately} succeed, in
	...
	sequential phase. Once all the workers are done accessing the state, the master thread will
	exclusively start using the state (essentially with ignoring all the taint values) and execute
...
we will bring this concept one step closer to reality by covering some of the implementation
details.
